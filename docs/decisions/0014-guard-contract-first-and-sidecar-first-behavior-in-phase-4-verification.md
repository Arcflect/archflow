...
Phase 4 established the first operational `archflow verify` command.
...
We formally define two invariants for `archflow verify`.
...
The following additions to `archflow verify` would violate this decision and require
...
Phase 4 では、最初の実用的な `archflow verify` コマンドを確立しました。
...
- [ADR-0007](./0007-archflow-remains-sidecar-first-before-code-aware-analysis.md): コード認識分析の前に sidecar ファースト
- [ADR-0013](./0013-first-version-verification-rule-boundaries.md): 初版 verification ルール境界

## 決定事項

`archflow verify` に対して 2 つの不変条件を正式に定義します。
...
`archflow verify` への以下の追加はこの決定に違反し、
